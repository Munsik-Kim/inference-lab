...
from src.common import CASE,sha,write_json,verify_spec,utc


def main():
    p=argparse.ArgumentParser();p.add_argument('--split',choices=['dev','confirmation'],required=True);p.add_argument('--snapshot',type=Path,required=True);p.add_argument('--work-dir',type=Path,required=True);a=p.parse_args()
    if a.split=='confirmation':spec,spec_sha=verify_spec()
    else:spec_sha=None
    import torch
    ...
    manifest=json.loads((CASE/'inputs/manifest.json').read_text());assert a.snapshot.name==manifest['revision']
    ...
    def capture(module,query,key,value,attention_mask,scaling,dropout=0.,**kwargs):
        ...
        doc=state['doc'];status='VALID' if error<=.01 else 'INVALID'
