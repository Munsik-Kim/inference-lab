...
## Structured report and same-record review

The [English report](REPORT.md#analysis) and [Korean report](REPORT.ko.md#analysis) add task-wise Q full/choice/mass NLL decomposition, constant-label diagnostics and code-generator cues. These are post-hoc calculations on the same retained records. They preserve the original primary full NLL and 32-position recovery metrics. R's smaller KL after repair and mixed gold-score changes are separate findings; no internal causal mechanism was isolated. [Recalculation and sources](publication/posthoc/metrics.json).
