...
def _horizon_statement(horizons, values, epsilon, max_horizon, confidence_bound=False):
    ...
    return {
        "epsilon": epsilon,
        "horizon": horizon,
        "quantity": "population_T_epsilon_lower_bound" if confidence_bound else "empirical_sample_T_epsilon",
        ...
        "interpretation": (
            f"simultaneous confidence-supported population lower bound T_epsilon >= {max_horizon}; finite observation does not imply infinity"
            if horizon == max_horizon and confidence_bound else
            f"empirical zero/few failures through Tmax={max_horizon}; population claim requires confidence_supported_T_epsilon_lower_bound"
            ...
            if confidence_bound else "largest qualifying empirical sample grid horizon; no interpolation or population guarantee"
        ),
    }
