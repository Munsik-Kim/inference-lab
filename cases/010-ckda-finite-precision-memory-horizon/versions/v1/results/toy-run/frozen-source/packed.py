...
ties-to-even; stochastic rounding uses SplitMix64(seed + counter + index)
...
class PackedCodec(_PayloadCodec):
    ...
    def __init__(self, shape: Any, bits: int = 4, mixed_bits: Any = None,
        ...
        self.config_bytes = _canonical_config({
            ...
            "rng": "splitmix64_seed_next_counter_le_u64" if self.stochastic else None,
    ...
    def _uniforms(self, seeds: np.ndarray, counters: np.ndarray) -> np.ndarray:
        with np.errstate(over="ignore"):
            z = seeds[:, None] + counters[:, None] + np.arange(self.num_values, dtype=np.uint64)
