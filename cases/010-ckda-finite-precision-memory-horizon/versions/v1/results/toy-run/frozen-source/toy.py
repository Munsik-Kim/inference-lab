...
def evaluate_arm(condition, arm, calibration, batch, family_size, horizons, alpha=0.05):
    ...
    for step in range(1, horizon + 1):
        ...
        state, output, diagnostics = adapter.step(state, affine_transition(operators))
        prototypes = condition.prototypes(step)
        predicted = readout(output, prototypes)
        correct[:, step - 1] = predicted == batch.gold[:, step - 1]
        values = output[:, 0, :, 0].astype(np.float64)
        reference = prototypes[batch.gold[:, step - 1]]
        ...
        if condition.group_name == "C31":
            ...
            reference_angle = batch.gold[:, step - 1] * (2 * math.pi / 31)
        ...
        if "projection_leakage_mse" in diagnostics:
            projection_leakage.append(float(diagnostics["projection_leakage_mse"]))
    ...
    diagnostics = {"state_mse_per_coordinate": squared_error / (n * horizon * condition.dimension),
                   "norm_mse": norm_error / (n * horizon),
                   "phase_mse_radians2": phase_error / (n * horizon) if condition.group_name == "C31" else None,
                   "mean_local_projection_leakage": float(np.mean(projection_leakage)) if projection_leakage else None,
                   "diagnostic_scope": "descriptive output-side trajectory diagnostics; no fitted failure prediction"}
    return {"condition": condition.name, "arm": arm, "split": batch.split, "status": "ok",
            "runtime_seconds": runtime, "summary": summary, "ledger": ledger, "diagnostics": diagnostics}, correct, shared_adapter + shared_model
