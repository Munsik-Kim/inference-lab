...
def render_case008(root: Path, data: dict, lang: str) -> str:
    ...
    h += '<nav class="actions" aria-label="'+text('tracks')+'">'+anchor('#track-q',t['qtitle'])+anchor('#track-r',t['rtitle'])+anchor('#try-it',t['try'])+'</nav>'
    for name,d in [('Q',q),('R',r)]:
        ...
        if isq:
            ...
            h += '<h3>'+text('scores')+'</h3><p>'+text('qscore')+'</p><dl class="score-list">'
            ...
            h += '</dl><p class="scope">'+text('nllScope')+'</p><p>'+text('qTiming')+' '+f'{tim["speedup"]:.3f}× [{tim["ci95"][0]:.3f}, {tim["ci95"][1]:.3f}]'+'</p><p class="scope">'+text('timeScope')+'</p>'
        else:
            ...
            h += '</div><p class="scope">'+text('rScope')+'</p><p>'+text('rConclusion')+'</p>'
